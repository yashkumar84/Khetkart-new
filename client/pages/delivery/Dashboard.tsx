      ...
      <div>
        ...
        <main className="container py-8">
          ...
          <div className="space-y-3">
            {rows.map((o) => (
              <div key={o._id} className="flex items-center justify-between rounded border p-3">
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" variant="secondary" onClick={async () => { await api(`/delivery/${o._id}/picked`, { method: "POST", auth: true }); load(); }}>Picked Up</Button>
                  <Button size="sm" onClick={async () => { await api(`/delivery/${o._id}/delivered`, { method: "POST", auth: true }); load(); }}>Delivered</Button>
                </div>
              </div>
            ))}
            {rows.length === 0 && <div className="text-muted-foreground">No assigned orders yet.</div>}
          </div>
        </main>
      </div>
    </ProtectedRoute>
